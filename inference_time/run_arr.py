"""
Runner script for ARR (Adaptive Regime Routing) inference-time evaluation.
...
from inference_time.arr import arr_generate, score_answers
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run ARR decoding on a probe set.",
    ...
    p.add_argument("--tau_jsd", type=float, default=0.05, help="JSD agreement threshold.")
    p.add_argument("--tau_ent", type=float, default=0.1, help="Entropy-diff routing threshold.")
    p.add_argument("--alpha_ctx", type=float, default=1.0, help="CAD strength for trust-context regime.")
    p.add_argument("--alpha_prior", type=float, default=1.0, help="CAD strength for trust-prior regime.")
    p.add_argument("--max_new_tokens", type=int, default=50)
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--top_p", type=float, default=1.0)
    p.add_argument("--repetition_penalty", type=float, default=1.0)
    p.add_argument("--results_dir", default="results")
    p.add_argument("--experiment_name", default=None, help="default: arr_default[_T{T}]")
...
def evaluate_probe(model, tokenizer, probe: dict, args, device) -> dict:
    ...
    if answers:
        scores = score_answers(
            ...
            tau_jsd=args.tau_jsd, tau_ent=args.tau_ent,
            alpha_ctx=args.alpha_ctx, alpha_prior=args.alpha_prior,
        ...
        regime_counts = dict(Counter(out["regimes"]))
        result["generated_text"] = out["generated_text"]
        result["regime_counts"] = regime_counts
        logger.info(
            "  [%s] generated: %s  regimes=%s",
            probe.get("id"), out["generated_text"][:80], regime_counts,
        )
...
def main():
    ...
    if args.experiment_name is None:
        args.experiment_name = default_experiment_name("arr_default", args)
    setup_logging(args.log_dir, args.experiment_name)
    logger.info("Starting ARR experiment: %s", args.experiment_name)
