"""
Runner script for CoCoA (confidence-aware contextual decoding) evaluation.
...
from inference_time.cocoa import cocoa_generate, score_answers
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run CoCoA decoding on a probe set.",
    ...
    p.add_argument("--beta", type=float, default=1.0, help="Global scale factor.")
    p.add_argument("--gamma_jsd", type=float, default=1/3, help="Weight for JSD signal.")
    p.add_argument("--gamma_ent", type=float, default=1/3, help="Weight for entropy-gap signal.")
    p.add_argument("--gamma_peak", type=float, default=1/3, help="Weight for peakedness signal.")
    p.add_argument("--max_new_tokens", type=int, default=50)
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--top_p", type=float, default=1.0)
    p.add_argument("--repetition_penalty", type=float, default=1.0)
    p.add_argument("--results_dir", default="results")
    p.add_argument("--experiment_name", default=None, help="default: cocoa_beta1_equal_weights[_T{T}]")
...
def evaluate_probe(model, tokenizer, probe: dict, args, device) -> dict:
    ...
    if answers:
        scores = score_answers(
            ...
            beta=args.beta, gamma_jsd=args.gamma_jsd,
            gamma_ent=args.gamma_ent, gamma_peak=args.gamma_peak,
...
def main():
    ...
    if args.experiment_name is None:
        args.experiment_name = default_experiment_name("cocoa_beta1_equal_weights", args)
    setup_logging(args.log_dir, args.experiment_name)
    logger.info("Starting CoCoA experiment: %s", args.experiment_name)
