"""
Runner script for COIECD (entropy-gated contrastive decoding) evaluation.

Probe JSON format: same as run_cad.py — each probe must have both "prompt"
(uncued) and "cued_prompt" (disambiguated).  Optional "answers" enables scoring mode.
...
from inference_time.coiecd import coiecd_generate, score_answers
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run COIECD decoding on a probe set.",
    ...
    p.add_argument("--alpha", type=float, default=1.0, help="CAD contrastive strength.")
    p.add_argument("--tau", type=float, default=0.1, help="Entropy-difference conflict threshold.")
    p.add_argument("--max_new_tokens", type=int, default=50)
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--top_p", type=float, default=1.0)
    p.add_argument("--repetition_penalty", type=float, default=1.0)
    p.add_argument("--results_dir", default="results")
    p.add_argument("--experiment_name", default=None, help="default: coiecd_alpha1_tau0.1[_T{T}]")
...
def evaluate_probe(model, tokenizer, probe: dict, args, device) -> dict:
    ...
    if answers:
        scores = score_answers(
            ...
            alpha=args.alpha, tau=args.tau, temperature=args.temperature, device=device,
        )
        ...
        result["conflict_flags"] = out["conflict_flags"]
        conflict_rate = sum(out["conflict_flags"]) / max(len(out["conflict_flags"]), 1)
        logger.info(
            "  [%s] generated: %s  (conflict_rate=%.2f)",
            probe.get("id"), out["generated_text"][:80], conflict_rate,
        )
...
def main():
    ...
    if args.experiment_name is None:
        args.experiment_name = default_experiment_name("coiecd_alpha1_tau0.1", args)
    setup_logging(args.log_dir, args.experiment_name)
    logger.info("Starting COIECD experiment: %s", args.experiment_name)
