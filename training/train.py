...
def build_optimizer(model, cfg) -> torch.optim.AdamW:
    ...
    return torch.optim.AdamW(param_groups, lr=cfg.lr, betas=(cfg.beta1, cfg.beta2), eps=cfg.eps)
...
def train(cfg: TrainingConfig, smoke_test: bool = False, build_sampler=None) -> None:
    ...
    val_loader = DataLoader(val_ds, batch_size=cfg.data.batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    start_step = 0
    if cfg.run.resume:
        start_step = ckpt_mgr.resume(model, optimizer, scheduler, device)
        if start_step > 0:
    ...
    for step in range(start_step + 1, cfg.optim.max_steps + 1):
        ...
        if step % cfg.run.log_interval_steps == 0:
            ...
            logger.info(f"step={step} loss={accum_loss:.4f} lr={lr_now:.3e} tok/s={tok_per_sec:.0f}")
            ckpt_mgr.log_metrics({"step": step, "loss": accum_loss, "lr": lr_now, "tokens_seen": step * tokens_per_step})
            t_last_log = time.time()
        ...
        ckpt_mgr.maybe_save(step, model, optimizer, scheduler, log_fn=logger.info)
